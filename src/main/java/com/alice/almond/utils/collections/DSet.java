package com.alice.almond.utils.collections;
...
public class DSet<T> implements Iterable<T> {
	public int size;
    ...
    private transient DSetIterator iterator1, iterator2;

    public DSet () {
		this(51, 0.8f);
	}

    public DSet (int initialCapacity) {
		this(initialCapacity, 0.8f);
	}


    public DSet (int initialCapacity, float loadFactor) {
    ...
    public DSet (DSet<? extends T> set) {
	...
	public boolean equals (Object obj) {
		if (!(obj instanceof DSet)) return false;
		DSet other = (DSet)obj;
	...
	public DSetIterator<T> iterator () {
		if (Collections.allocateIterators) return new DSetIterator(this);
		if (iterator1 == null) {
			iterator1 = new DSetIterator(this);
			iterator2 = new DSetIterator(this);
		}
		if (!iterator1.valid) {
			iterator1.reset();
	...
	static public <T> DSet<T> with (T... array) {
		DSet<T> set = new DSet<T>();
		set.addAll(array);
		return set;
	}
	...
	static public class DSetIterator<K> implements Iterable<K>, Iterator<K> {
		...
		public DSetIterator (DSet<K> set) {
			this.set = set;
			reset();
		}
		...
		public DSetIterator<K> iterator () {
			return this;
		}
